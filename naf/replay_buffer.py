...
class ReplayBuffer:
    ...
    def __init__(self, buffer_size, batch_size, device, seed, gamma, nstep):
        ...
        self.experience = namedtuple("Experience", field_names=["state", "action", "reward", "next_state", "done", "t"])
    ...
    def add(self, state, action, reward, next_state, done, t):
        ...
        self.n_step_buffer.append((state, action, reward, next_state, done, t))
        if len(self.n_step_buffer) == self.n_step:
            state, action, reward, next_state, done, t = self.calc_multistep_return()

            e = self.experience(state, action, reward, next_state, done, t)
    ...
    def calc_multistep_return(self):
        ...
        return self.n_step_buffer[0][0], self.n_step_buffer[0][1], Return, self.n_step_buffer[-1][3], self.n_step_buffer[-1][4], self.n_step_buffer[0][5]
    ...
    def sample(self):
        ...
        ts = torch.from_numpy(np.vstack([e.t for e in experiences if e is not None]).astype(np.int32)).float().to(self.device)
  
        return (states, actions, rewards, next_states, dones, ts)
...
class ModelingBuffer:
    """Fixed-size buffer to store experience tuples for fiting ilqg."""

    def __init__(self, buffer_size, batch_size, device):
        """Initialize a ReplayBuffer object.
        Params
        ======
            buffer_size (int): maximum size of buffer, should be n * T where n is a hyperparam.
                               Authors recommend n = 5.
        """
        self.batch_size = batch_size
        self.device = device
    ...
    def add(self, state, action, reward, next_state, done, t):
        """Add a new experience to memory from constituent components."""
        e = self.experience(state, action, reward, next_state, done, t)
        self.memory.append(e)
    ...
    def sample_exps(self, get_all=False):
        """Randomly sample a batch of experiences from memory."""
        k = len(self.memory) if get_all else self.batch_size
